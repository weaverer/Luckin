...
4. **Generate tasks.md**: Read the tasks template from TASKS_TEMPLATE (from the JSON output above) and use it as structure. If TASKS_TEMPLATE is empty, fall back to `.specify/templates/tasks-template.md`. Fill with:
   ...
   - Each phase includes: story goal, independent test criteria, constitution-required tests,
...
**Tests are REQUIRED when applicable under the constitution**: Generate test tasks for changes
to observable behavior, public contracts, data models, or failure handling. Defect fixes require
a failing regression test first. If tests are genuinely not applicable, record a concrete reason.

For every third-party data API, generate separate tasks for the provider-neutral port and canonical
...
1. **From User Stories (spec.md)** - PRIMARY ORGANIZATION:
   ...
   - Map all related components to their story:
     ...
     - Constitution-required tests specific to that story
   - Mark story dependencies (most stories should be independent)

2. **From Contracts**:
   - Map each interface contract → to the user story it serves
   - Each changed interface contract → contract test task [P] before implementation in that story's phase
   - Each third-party data contract → provider-neutral port/model, adapter, provider selection,
     error mapping, and replacement-adapter or test-double verification tasks
