...
运行四个接口：复权因子 9:30（开盘后获取前一交易日）、日线 17:00、基本面指标 17:45、
...
（18:30 启动）同步当日形成终态，复权因子（9:30 启动）开盘后获取前一交易日因子；
单日全市场约 5,400 行、单次请求容纳，同步分钟级完成；
2024-01-01 起的回补（约 610 个交易日）在来源限流约束内逐日完成，可中断恢复

**约束**：

- 计划 Cron 固定为复权因子 `30 9 * * 1-5`、日线 `0 17 * * 1-5`、基本面 `45 17 * * 1-5`、
...
1. 新增 `market-data-sync` 参数化 Flow 与五个 Deployment：
   `复权因子同步`（Cron `30 9 * * 1-5`）、`日线行情同步`（Cron `0 17 * * 1-5`）、
   `每日基本面同步`（Cron `45 17 * * 1-5`）、`周K线同步`与`月K线同步`（Cron
   `30 18 * * 1-5`，各自独立运行），
