...
4. ✅ 已实测（2026-08-06/08-07）：账户级共享限流（RedisRateLimiter
   400/min）全程未出现限流拒绝——top10 空表 612 天积压窗口 + 网络错误
   （`PROVIDER_UNAVAILABLE` 3 次、退避 30/120/300 秒）于 1500 秒截止
   触发 `PROVIDER_DEADLINE`，非限流错误；限流档位错误码映射沿用
   Adapter 契约（`PROVIDER_RATE_LIMITED`），重试退避 30/120/300 秒
   由契约测试覆盖（T008）。
...
6. 实测各接口回补的请求量与耗时（3 Flow 拆分后按接口独立回补：
   ...
   确认各接口在回补窗口内可行。
7. ✅ 已实测（2026-08-06）：更正公告收敛形态——同一披露期（2026-06-30）
   在**同一公告日**（08-04）重复披露且数值略异（300852.SZ 四会富仕
   温一峰 3709894.0 vs 3709912.0）：按同日重复披露隔离
   （`DUPLICATE_ANN_DISCLOSURE`，保留首见、后见计 `invalid_count`），
   不整批失败；跨日更正公告（入站 ann_date 更大）由 updated 逻辑按最新
   值收敛（契约测试 T009 持续覆盖）。
8. ✅ 已实测（2026-08-06/08-07）：三接口错峰调度（17:00/17:05/17:10）
   串行执行无叠加——08-06：17:00 top10 启动（超时失败）、17:05 float
   启动、17:10 count 启动，三 run 时间线无并发；08-07：三 run 均
   COMPLETED；账户级限流由 RedisRateLimiter 跨进程强保证（待验证项 4）。
