...
- **测试与质量门禁**：是否定义单元、契约、集成及关键端到端测试，以及适用的静态检查、
  类型检查和构建命令？
- **统一公共 API 响应**：如功能新增或修改公共 JSON API，是否定义并验证
  `code/message/data/errors/request_id/timestamp` 顶层结构、成功与失败业务码、HTTP 状态与
  业务码分离、`data.items/data.pagination` 分页结构，以及 OpenAPI 强类型响应？
