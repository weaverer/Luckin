"""Tushare ``adj_factor`` 日线复权因子 Adapter。"""
...
class TushareAdjFactorProvider:
    ...
    def __init__(
        ...
        *,
        ...
        if page_limit != 6000:
            raise ValueError("Tushare adj_factor 页面上限固定为 6000")
        if max_pages <= 0:
    ...
    def fetch_adj_factors(
        self, request: AdjFactorRequest, *, deadline: float
    ) -> ProviderAdjFactorBatch:
        ...
        for row in rows:
            try:
                ...
                raise ProviderPayloadError(
                    self.provider_code, f"复权因子记录无效：{exc}"
                ) from exc
...
def _factor(value: Any) -> Decimal:
    """来源 adj_factor 实测返回 float，兼容字符串与 Decimal。"""
    if isinstance(value, str):
        if not value.strip():
            raise ValueError("adj_factor 为空或类型非法")
        try:
            return Decimal(value.strip())
        except InvalidOperation as exc:
            raise ValueError("adj_factor 不是有效数字") from exc
